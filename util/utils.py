...
def get_val_data_cfp(data_path):
    cfp_ff, cfp_ff_issame = get_val_pair(data_path, 'cfp_ff')
    cfp_fp, cfp_fp_issame = get_val_pair(data_path, 'cfp_fp')
    return cfp_ff, cfp_ff_issame, cfp_fp, cfp_fp_issame

def get_val_data_cfp_fp(data_path):
    cfp_fp, cfp_fp_issame = get_val_pair(data_path, 'cfp_fp')
    return cfp_fp, cfp_fp_issame

def get_val_data_lfw(data_path):
...
def get_test_data(data_path):
    ...
    cplfw, cplfw_issame = get_val_pair(os.path.join(data_path, 'cplfw_align_112'), 'cplfw')
    vgg2_fp, vgg2_fp_issame = get_val_pair(os.path.join(data_path, 'VGGFace2_FP'), 'vgg2_fp')
    agedb_30, agedb_30_issame = get_val_pair(os.path.join(data_path, 'AgeDB'), 'agedb_30')
    calfw, calfw_issame = get_val_pair(os.path.join(data_path, 'CALFW'), 'calfw')
    return lfw, lfw_issame, cfp_ff, cfp_ff_issame, cfp_fp, cfp_fp_issame, cplfw, cplfw_issame, vgg2_fp, vgg2_fp_issame, agedb_30, agedb_30_issame, calfw, calfw_issame
...
def cosine_loss(inputs, targets): # input, target: (classes * 512)
    loss = 1 - torch.cosine_similarity(inputs, targets) # (classes)
    return loss.mean()
